...
echo ""
echo "=== Flags with value arguments ==="
assert_eq "git -C path"             "git -C /some/long/path status"                     "Running git status."
assert_eq "git -C long path"        "git -C /Users/me/projects/repo push"               "Running git push."
assert_eq "git commit -m msg"       "git commit -m 'initial commit'"                    "Running git commit."
assert_eq "long flag with ="        "git log --format=oneline"                          "Running git log."
assert_eq "long flag with value"    "git clone --depth 1 https://example.com"           "Running git clone."
assert_eq "multiple long flags"     "docker run --name myapp --rm image"                "Running docker run."
assert_eq "mixed short and long"    "curl -X POST --header auth https://api.com"        "Running curl."
assert_eq "rsync with -e flag"      "rsync -e ssh src/ dest/"                           "Running rsync."
